...
def process_new_decisions():
    ...
    for decision in result.data:
        print(f"[SYNC] New decision detected: {decision['match_id']}")

        # Step 1: Encode the payload
        payload_hex = encode_wirefluid_payload(decision)
        print(f"[OK] Encoded payload: {payload_hex[:40]}...")
        ...
        print(f"[OK] Supabase updated to 'encoded'")
...
if __name__ == "__main__":
    print("[START] Execution trigger running... polling every 10 seconds")
    while True:
